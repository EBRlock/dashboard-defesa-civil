...
if not df.empty:
    ...
    with col_sup_dir:
        ...
        for idx, row in df_filtrado.iterrows():
            try:
                ...
                tipo_bruto = str(row.get('tipo', 'Ocorrência'))
    ...
    with col_inf_esq:
        ...
        with pizza_col:
            ...
            fig_pie.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=180,
                                  ...
                                  font=dict(color='black'), showlegend=True)
    ...
    with col_inf_dir:
        ...
        if not df_grafico.empty:
            ...
            contagem_mes = contagem_mes.sort_values(by='Mês') 
            
            fig_bar = px.bar(contagem_mes, x='Mês', y='Quantidade', text='Quantidade')
            fig_bar.update_traces(marker_color='#191970', textposition='outside') 
            
            fig_bar.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=180, 
                                  paper_bgcolor='white', plot_bgcolor='white',
                                  font=dict(color='black'), xaxis_title="Mês", yaxis_title=None)
