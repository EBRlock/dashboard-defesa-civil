    ...
    try:
        ...
        if dados:
            ...
            colunas_padrao = {'tipo': 'Não Informado', 'encaminhamento': 'Não Informado',
                              'risco': 'Médio', 'data': 'Recente', 'bairro': 'Não Informado', 'municipio': 'Manaus'}
...
if not df.empty:
    ...
    # ==========================================
    st.markdown(
        "<div class='cabecalho'>PAINEL DE ATENDIMENTO DO CALL CENTER - DEFESA CIVIL DO ESTADO DO AMAZONAS</div>",
        unsafe_allow_html=True)
    ...
    filtro_ano = col_f4.selectbox("ANO", ["Todos"])
    filtro_mes = col_f5.selectbox("MÊS", ["Todos"])
    ...
    with col_sup_esq:
        ...
        with tab1:
            st.markdown(
                "<div style='font-size: 13px; font-weight: bold; margin-bottom: 5px; color: #555;'>NATUREZA</div>",
                unsafe_allow_html=True)
        ...
        with tab2:
            st.markdown(
                "<div style='font-size: 13px; font-weight: bold; margin-bottom: 5px; color: #555;'>ENCAMINHAMENTO</div>",
                unsafe_allow_html=True)
    ...
    with col_sup_dir:
        ...
        for idx, row in df_filtrado.iterrows():
            try:
                ...
                # ==========================================
                # NOVA LÓGICA: COR BASEADA NA NATUREZA (TIPO)
                # ==========================================
                ...
                if 'INCÊNDIO' in tipo_upper:
                    ...
                    cor_icone, cor_hex = 'cadetblue', '#607D8B'  # Cor padrão para Outros
    ...
    with col_inf_esq:
        ...
        with metrica_col:
            st.markdown("<div style='font-size: 13px; font-weight: bold; color: #555;'>Total de Registros</div>",
                        unsafe_allow_html=True)
            st.metric("", len(df_filtrado))

        with pizza_col:
            st.markdown(
                "<div style='font-size: 13px; font-weight: bold; color: #555; text-align: center;'>Nível de Risco</div>",
                unsafe_allow_html=True)
    ...
    with col_inf_dir:
        st.markdown("<div style='font-size: 13px; font-weight: bold; color: #555;'>Registros por Dia/Mês</div>",
                    unsafe_allow_html=True)
        if 'data' in df_filtrado.columns:
            contagem_mes = df_filtrado['data'].value_counts().reset_index()
            contagem_mes.columns = ['Data', 'Quantidade']

            fig_bar = px.bar(contagem_mes, x='Data', y='Quantidade')
